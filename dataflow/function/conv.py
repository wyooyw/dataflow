...
def conv_backward(output_grad:np.ndarray,input:np.ndarray,weight:np.ndarray):
    """
    卷积反传
    分别计算输入梯度和权重梯度
    TODO:分块
...
def calc_input_grad(output_grad,weight):
    ...
    input_grad = S2TrainInterface.execute_conv(input=output_grad,weight=weight, \
...
def calc_weight_grad(output_grad,input):
    ...
    weight_grad = S2TrainInterface.execute_conv(input=input,weight=output_grad, \
                                                    in_channels=in_channels, \
                                                    out_channels=out_channels, \
                                                    kernel_size=kernel_size,
                                                    bias=False)
