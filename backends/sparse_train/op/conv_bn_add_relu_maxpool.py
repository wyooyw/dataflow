...
class ForwardConvBnAddReluMaxpool(Operator):
    ...
    def to_instr(self):
        ...
        instruction.set("pool_mask",int_to_bits(self.tensors.get("maxpool.mask").index,9).to01(),use_bits=True)
        instruction.set("res_add",int_to_bits(self.tensors.get("add").index,9).to01(),use_bits=True)
    ...
    def replace_from(self,find_ops):
        ...
        """
        conv,bn,add,relu,maxpool = find_ops
        return ForwardConvBnAddReluMaxpool(conv=conv,bn=bn,add=add,relu=relu,maxpool=maxpool)